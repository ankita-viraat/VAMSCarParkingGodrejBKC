...
import info.vams.zktecoedu.reception.Model.ParkingModels.ParkCheckout.ParkCheckOutResp;
...
public class PostParkCheckOutHelper {
    public static void callMaster(final Context context, final IGetResponse iGetResponse, ParkCheckOutReq parkCheckOutReq) {
        ...
        Call<ResponseBody> call = ApiUtils.getAPIService().postParkingCheckOut(Utilities.getToken(context),
                parkCheckOutReq );
                ...
                if (response.code() == 200 || response.code() == 201) {
                    if (response != null) {
                        try {
                            ...
                            ParkCheckOutResp parkCheckOutResp = new Gson().fromJson(responseStr,
                                    new TypeToken<ParkCheckOutResp>() {}.getType());
                            Utilities.hideProgress();
                            iGetResponse.isSuccess(true, parkCheckOutResp);
                        } catch (Exception e) {
                            //write log method
                            Utilities.hideProgress();
