...
public class GetEmployeeHelper {
    public static void callHelper(final Context context, GetParkEmployeeReq parkEmployeeReq, final IGetResponse iGetResponse) {
        ...
        Call<ResponseBody> call = ApiUtils.getAPIService().getParkingEmployee(Utilities.getToken(context),
                parkEmployeeReq);
        call.enqueue(new Callback<ResponseBody>() {
            ...
            public void onResponse(Call<ResponseBody> call, Response<ResponseBody> response) {
                ...
                if (response.code() == 200 || response.code() == 201) {
                    if (response != null) {
                        try {
                            String responseStr = new JSONObject(response.body().string().toString()).getString("results");
                            ArrayList<GetParkEmployeeResp> getParkEmployeeResps = new Gson().fromJson(responseStr,
                                    new TypeToken<ArrayList<GetParkEmployeeResp>>() {
                                    }.getType());
                            Utilities.hideProgress();
                            iGetResponse.isSuccess(true, getParkEmployeeResps);
                        } catch (Exception e) {
                            //write log method
                            Utilities.hideProgress();
